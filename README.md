...
El SDK oficial usa `handleUploadPresigned` en `/api/audio/upload` y `uploadPresigned` de `@vercel/blob/client` en `transcribeAudio`. Una primera petición prepara el pathname y `audioRef`; el SDK solicita autorización al mismo endpoint con esos datos, y el servidor comprueba que ambos coincidan. Dentro de `getSignedToken`, `issueSignedToken` autoriza: solo PUT, store privado, pathname exacto, MIME permitido, tamaño máximo igual al declarado (hasta 24 MB), vigencia de 3 minutos y `allowOverwrite: false`. El SDK recibe el permiso firmado restringido, nunca el material privado de firma ni las credenciales del store. El permiso es temporal y confidencial: puede repetirse hasta vencer si el archivo ya se eliminó; no es consumible una sola vez. La aplicación genera otra referencia al reintentar.

`uploadPresigned` realiza la subida directamente a Vercel Blob y gestiona los headers del protocolo. No hay PUT ni firmado de URL manual en la aplicación. El cliente verifica el pathname devuelto por el SDK. Luego `POST /api/transcribe` recibe `{ audioRef: { key, extension } }`, validado con Zod y limitado a 4 KB. El servidor deriva el pathname dentro de su propio store, lee con `get(..., { access: "private", useCache: false })`, limita los bytes, valida MIME/firma de audio y lo envía a Gemini Files API. El pathname no permite derivar la referencia aleatoria que autoriza procesamiento y borrado.
...
- El store debe ser privado y estar conectado al entorno del despliegue. El SDK resuelve OIDC y `BLOB_STORE_ID` inyectados por Vercel. `handleUploadPresigned` 2.8.0 también requiere `BLOB_WEBHOOK_PUBLIC_KEY` de la conexión, incluso sin registrar callbacks. No se agrega token manual. Las credenciales del dashboard no aparecen automáticamente al ejecutar `next dev` localmente.
